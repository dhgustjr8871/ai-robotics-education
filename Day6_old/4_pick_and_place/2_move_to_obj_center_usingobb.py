...
if __name__ == "__main__":
    ...
    try:
        ...
        cx = int(cx/2)
        cy = int(cy/2)
        angle = min(angle, angle-np.pi/2)  # Adjust angle to be within -pi/2 to pi/2
        # 3D 위치 계산
        ...
        if pos_3d is None:
            ...
            print(f"{target_name} 중심의 픽셀 위치 (CX, CY): {np.round(cx, 4)} {np.round(cy, 4)}")
            print(f"{target_name} 각도: {np.round(angle, 4)}")
